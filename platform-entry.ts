...
function stats(env: Env) {
  return {
    ...
    telemetry: {
      ...
      note: "Counters are intentionally process-local and reset when the Worker isolate is recycled. No credentials, cookies, prompts, tokens or private account data are exposed.",
    },
  };
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Validate browser origins before handing the request to the MCP handler.
    // This lets the Platform use the Worker without opening MCP to arbitrary web origins.
    if (origin && !allowedOrigin(origin, env)) {
      return new Response("Forbidden origin", { status: 403, headers: { "Vary": "Origin" } });
    }

    if (request.method === "OPTIONS") {
      if (origin && !allowedOrigin(origin, env)) return new Response("Forbidden origin", { status: 403 });
    ...
    // Strip the already-validated browser Origin before delegating to the legacy MCP
    // handler. The wrapper is the origin gate; this avoids the old handler rejecting
    // the future Platform origin before it can reach the MCP endpoint.
